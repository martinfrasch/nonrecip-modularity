...
All code, per-run summaries and figures are in the repository \texttt{github.com/martinfrasch/nonrecip-modularity}, organised as \texttt{sim/} (model and integrators), \texttt{analysis/} (measurements), \texttt{external/} (comparison systems and their open data), \texttt{figures/}, \texttt{paper/} (this manuscript and its LaTeX build), \texttt{results/} (per-run summaries as CSV, the pilot and baseline outputs) and \texttt{notes/} (working notes and per-analysis records). Scripts run as modules from the repository root, e.g. \texttt{python -m analysis.static\_epr}.
