...
Nonreciprocal interactions have no scalar potential, and a common intuition splits such a
coupling into a symmetric sector that selects structure and an antisymmetric one that only adds
circulation. A directed-graph extension of the Network-Weighted Action Principle (minAction.net)
rests on that split, and we test it. In the colloid model of Hara et al. [Phys. Rev. Lett.
**137**, 068302 (2026)] a mixing parameter χ scales the antisymmetric part of the pair force with
the symmetric part unchanged; χ = 0 restores Newton's third law and χ = 1 the published law. Across 113 blocked-design runs the antisymmetric sector alters equal-time structure by an order of magnitude and does not preserve the reciprocal
stationary density, refuting the solenoidal picture: it unjams the reciprocal gel at weak drive
and sustains a fragment population at strong drive, so cluster count is non-monotonic in χ. The
fragmented state is a long-lived transient (largest cluster ∝ N^1.00±0.07, condensation time
∝ N^1.0), and the selected scale of 8–10 particles is a critical-size crossover with a
horizon-converged committor of one half. Excess dissipation over the reciprocal control, from a
configurational estimator, is unresolved at χ ≤ 0.25 and quadratic from
χ = 0.75; the symmetric forces cancel 87–91 % of the unopposed antisymmetric contribution above
that crossover. The fragment population is a kinetic balance of approximately balanced channel pairs. On single-particle coordinates one effective temperature, equal to the bath's, is
shared by both species over a short-lag window that shrinks with drive. No circulation is
detectable in any coarse observable examined, and Newman modularity cannot detect nonreciprocity,
so the modularity-excess prediction fails with inverted sign. Axonemes, a chiral flock and dominance
matrices bound how far the circulation result generalises.
...
The reason to pursue that hypothesis at all is the programme it belongs to. The Network-Weighted
Action Principle is the physical core of minAction.net, a programme asking whether living
organisation — physiological regulation [9], the modular structure of metabolic networks [30],
and learned models of physical systems [31, 32] — can be read as minimising an action that trades
energy against information under the cost of forming and maintaining connections, S_NW =
∫(E − I + A·C) dt [9, 33]. Its published tests are meta-analytic or computational. A framework of
that kind needs at least one system in which a single knob varies the quantity it singles out and
a prediction can fail cleanly, and nonreciprocal colloids are the first physical system we know of
in which the antisymmetric sector can be scaled alone with everything else held fixed. That is
what this paper does. The outcome, reported by the framework's author as a test of his own
hypothesis, is that the decomposition survives as an instrument and the mechanism assigned to it
does not; the sections that follow say which predictions failed, which held, and what the
framework would have to reproduce to account for the structure the sector selects.
...
Simulation (`sim/`): `simulate.py` (model, reciprocity parameter, sweeps), `simulate_par.py`
(thread-parallel force kernel, validated to 8.9 × 10⁻¹⁵ against the serial kernel),
`simulate_rect.py` (rectangular box, bitwise identical to the square kernel at L_x = L_y),
`continue_run.py` (run continuation, dense continuations). Analysis (`analysis/`): `analyze.py`
(network observables), `static_epr.py` (configurational excess dissipation with small-timestep
calibration), `epr_probe.py` (paired trajectory probe), `cluster_kinetics.py` (event
classification, channel-resolved drift, condensate shedding and reabsorption),
`fragment_budget.py` (source–sink budget), `committor.py`, `hs_fdt.py` and `hs_analysis.py`
(random-sign fluctuation–response twins and the single-particle effective temperature),
`fdt.py`, `fdt_driftfree.py` and `drift_control.py` (species-coordinate response and the
centre-of-mass drift), `onsager.py`, `onsager_equil.py` and `onsager_equil2.py` (cross-response),
...
(all-pairs lag asymmetry against block-shuffled surrogates), `arrest_test.py`
(configuration-swap test), `crossover.py` (per-cluster propulsive force),
`single_cluster_circulation.py`, `irreversibility.py` and `benchmark_irrev.py` (time-series
estimators and their calibration against an exactly solvable biased ring walk). Comparison
systems (`external/`): `cilia_analysis.py`, `vicsek.py`, `hodge_dominance.py`,
`forkosh_asymmetry.py`, with the published dominance matrices and behavioural table. Figures:
`figures/make_figures.py`. Validation: `tests/test_reciprocity.py` (the χ construction).

Supplementary figures, not reproduced here (`figures/`): `modularity_test.png` (baseline
four-panel), `snapshots.png` (final-frame renders), `coarsening.png` (trajectories by χ),
`chi_test.png`, and `sweep.png` (the coupling-strength and size-ratio sweeps of Section 3.1).

Supporting documents: `paper/EXPERIMENT.md` (pre-registered protocol with thresholds committed
before running), `paper/MANUSCRIPT_AUDIT.md` (the internal review rounds and every correction),
and in `notes/`: `AUDIT.md` (observable validity), `BOX_SCALING.md`, `COARSENING.md`,
`ONSAGER_RESULT.md`, `CILIA_RESULT.md`, `DOMINANCE_HODGE_RESULT.md`, `FORKOSH_RESULT.md`,
`VICSEK_RESULT.md`, `VARIATIONAL_FAMILY.md`, `OPEN_QUESTIONS_PLAN.md` and
`RESULTS_OPEN_QUESTIONS.md` (the programme behind Sections 3.4–3.10 and its numbers),
`FOLLOWUP.md`, `TODO.md`.
