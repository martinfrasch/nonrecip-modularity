...
All code, per-run summaries and figures are in the repository \texttt{github.com/martinfrasch/nonrecip-modularity}. Simulation: \texttt{simulate.py} (model, reciprocity parameter, sweeps), \texttt{simulate\_par.py} (thread-parallel force kernel, validated to 8.9 $\times$ 10$^{-15}$ against the serial kernel), \texttt{simulate\_rect.py} (rectangular box, bitwise identical to the square kernel at $L_x$ = $L_y$), \texttt{continue\_run.py} (run continuation). Analysis: \texttt{analyze.py} (network observables), \texttt{epr\_probe.py} (entropy production), \texttt{onsager.py}, \texttt{onsager\_equil.py} and \texttt{onsager\_equil2.py} (cross-response), \texttt{fdt.py} (effective temperature), \texttt{arrest\_test.py} (configuration-swap test), \texttt{crossover.py} (per-cluster propulsive force), \texttt{irreversibility.py} and \texttt{benchmark\_irrev.py} (time-series estimators and their calibration against an exactly solvable biased ring walk). Comparison systems: \texttt{cilia\_analysis.py}, \texttt{single\_cluster\_circulation.py}, \texttt{vicsek.py}, \texttt{hodge\_dominance.py}, \texttt{forkosh\_asymmetry.py}. Controls: \texttt{drift\_control.py} (whether the collective-coordinate superdiffusion is whole-system drift) and \texttt{fdt\_driftfree.py} (effective temperature with and without that drift, from the same starts). Kinetics, fates, response and dissipation: \texttt{static\_epr.py} (configurational excess dissipation with small-timestep calibration), \texttt{cluster\_kinetics.py} (event classification, Becker--D\"oring drift, condensate shedding and reabsorption), \texttt{committor.py}, \texttt{hs\_fdt.py} and \texttt{hs\_analysis.py} (random-sign fluctuation--response twins and the single-particle effective temperature), \texttt{coarsening\_law.py} (structure-factor length, mass-weighted cluster size, chained continuations), \texttt{hodge\_contact.py} (directed contact graph and its Hodge decomposition), \texttt{lag\_asymmetry.py} (all-pairs lag asymmetry against block-shuffled surrogates); \texttt{OPEN\_QUESTIONS\_PLAN.md} and \texttt{RESULTS\_OPEN\_QUESTIONS.md} record the programme and its numbers. Figures: \texttt{make\_figures.py}. Validation: \texttt{tests/test\_reciprocity.py} (the $\chi$ construction).

Supplementary figures, not reproduced here: \texttt{modularity\_test.png} (baseline four-panel), \texttt{snapshots.png} (final-frame renders), \texttt{coarsening.png} (trajectories by $\chi$), \texttt{chi\_test.png}, and \texttt{sweep.png} (the coupling-strength and size-ratio sweeps of Section 3.1).
