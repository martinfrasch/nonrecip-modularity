...
Three features of the table carry the argument of Section 3.10. The excess is zero within error at $\chi$ = 0.25 (each of three seeds within 1$\sigma$ of zero), a third of its plateau value at $\chi$ = 0.5, and on the plateau of 2.3--3.3 $\times$ 10$^{-3}$ per $\chi ^{2}$ from $\chi$ = 0.75 to $\chi$ = 8. The unopposed term $\tilde a$ is nearly constant in $\chi$ (0.025--0.030; it counts L--S contacts, not drive), so the entire $\chi$-dependence sits in the screening fraction, which is 1.0 below the threshold and 0.87--0.91 above it: nine tenths of the antisymmetric push is balanced by contact forces at every drive from 0.75 to 8, and all of it below 0.5. At $N$ = 1000, 9000 and 16 000 the plateau per $\chi ^{2}$ is 3.7, 2.4 and 2.4 $\times$ 10$^{-3}$ at $\chi$ = 1.5, so the pilot-scale value is about 30 \% high and the larger boxes agree with $N$ = 4000.
...
\textbf{The dip has a quantitative account.} Dense continuations ($\Delta$t = 50, three seeds per $\chi$) resolve the individual events that make and remove fragments --- clusters of size $\ge$ 2 other than the condensate. Tracking every cluster to its plurality successor, the condensate's \emph{shedding rate} (pieces of size $\ge$ 2 detaching per interval) and the fragments' \emph{reabsorption probability} (per fragment per interval, successor is the condensate) are:
...
At every $\chi$, including the reciprocal reference, the fragment count is the shedding rate divided by the reabsorption probability to within 8 \%. The non-monotonic curve is the product of two monotone rates with different thresholds. Reabsorption switches on at $\chi$ = 0.25 --- eleven times its reciprocal value, because weak activity mobilises fragments and condensate alike --- while shedding is still at its reciprocal level, so fragments are cleared faster than they are made and the cluster count falls fivefold. Shedding then switches on between $\chi$ = 0.25 and 0.5 (eightfold) and again to 0.75 (threefold), the fission probability of an existing fragment rises from zero to 0.30 over the same interval, and the fragment count climbs two orders of magnitude. Structure is a rate-balance fixed point. We note for Section 3.10 that the shedding threshold, between $\chi$ = 0.25 and 0.75, is where the excess dissipation of Section 3.4 switches on.
...
\subsection{The selected scale is a critical nucleus, not a stable cluster size}
...
\textbf{The size drift changes sign.} Restricting to monomer and dimer exchange --- the Becker--D\"oring channel, with fission and fusion excluded --- the mean size change per interval is
...
negative from $S$ = 3 to about 9 and positive above, with a zero at \textbf{$S^*$ = 10.1} (68 \% block- bootstrap interval 9.7--10.7, defined in every resample). Absorption into the condensate, kept separate, is a small flat hazard of 1.5--3.5 \% per interval at every size; folded into the drift it returns condensate size at every $S$, which is the artefact the coarse-interval attempt had produced. Dimers are the one exception to the sign pattern and gain on net.

\textbf{The committor is one half there.} From the same configurations, 300 relaunches with fresh noise ($\hat{t}$ = 1000) followed 23 800 fragments to the first of: reaching $S$ $\ge$ 20, falling to $S$ $\le$ 2, or being absorbed. The probability of the condensed outcome, q($S_0$) = P(grow $\cup$ absorbed) among decided fates, is

\begin{table}[htbp]
\caption{Committor of fragment clusters at $\chi=1.5$ from 300 relaunches with fresh noise, with and without absorption into the condensate counted as the condensed outcome.}
\label{tab:9}
\begin{ruledtabular}
...
still undecided at $\hat{t}$ = 1000 & 0.10 & 0.15 & 0.23 & 0.33 & 0.29 & 0.42 & 0.55 & 0.41 & 0.28 & 0.03 \\
\end{tabular}
\end{ruledtabular}
\end{table}

with binomial errors of 0.01--0.02 per entry. q is monotone in $S_0$ and crosses $1/2$ at $S_0$ = 8.0 (8.8 with absorption excluded), one bin below the drift zero; the fraction still undecided after $\hat{t}$ = 1000 peaks at $S_0$ = 8--12, where clusters linger longest, which is the signature of a saddle. Two independent definitions of the critical size therefore agree to within a bin: \textbf{$S^*$ $\approx$ 8--10 particles is a critical nucleus in the standard sense}, an unstable fixed point of the size dynamics at which the committor is one half. It is a property of the contact energetics rather than of the drive: the drift zero is 10.7, 9.3 and 10.1 at $\chi$ = 0.75, 1 and 1.5 (7.3 at $\chi$ = 0.5, with a tenth of the events), and Section 3.8 has already shown it independent of $N$. The fragment population (mean $\approx$ 5) sits below it and is, in the usual language, subcritical.
...
The state is thus a condensate coexisting with a subcritical population of small clusters that form by shedding and dissolve by evaporation, with the nucleus at $S^*$ $\approx$ 8--10 separating the two. No mechanism caps cluster size, consistent with \S{}3.8: anything above $S^*$ grows, the largest cluster grows as $N^{1.00\pm0.07}$, and the system phase-separates.
...
\emph{The construction's cross-term is contact screening, and it is quadratic, not linear.} An earlier version of this work fitted the low-drive data as a$\chi ^{2}$ + b$\chi$ and reported a negative linear term resolved at 5.6$\sigma$. That form cannot be right at the origin: the excess is non-negative for every $\chi$ and zero at $\chi$ = 0, so its slope at $\chi$ = 0 cannot be negative, and the fitted form goes negative below $\chi$ = 0.45. The configurational decomposition shows what the fit was describing. The unopposed term $\tilde a$ is nearly constant in $\chi$; $\tilde b$ is proportional to $- \chi$$\tilde a$ with coefficient 0.87--0.91 at every drive from $\chi$ = 0.75 to 8, and with coefficient 1.0 at $\chi$ $\le$ 0.5. The ``linear term'' is a quadratic structural screening: $\tilde b$ vanishes identically in a Boltzmann ensemble, and its measured value at $\chi$ = 0 is indeed zero within noise (+0.0003 on $\tilde a$ = 0.030), so it is a response of the structure to the drive and not a property of the reference. Nine tenths of the antisymmetric push is balanced by contact forces above the threshold, and all of it below.

\emph{The threshold is structural.} The screening fraction steps from 1.0 to 0.90 between $\chi$ = 0.25 and 0.75, which is exactly where Section 3.5 puts the onset of condensate shedding and fragment fission. Below it the condensate is a jammed solid in which the antisymmetric push is stored as contact stress and neither fragments nor dissipates; above it the contacts yield, the condensate sheds, and the residue of the push that the contacts no longer hold is what dissipates. The measurement is therefore best read as establishing that dissipation and fragmentation share one threshold, and as a validation of the configurational estimator against the trajectory probe across a hundredfold range of signal, rather than as an independent test of linear response: above the threshold the quadratic form is built into the construction, and below it there is nothing to fit.
...
The estimator calibrates exactly: at $\chi$ = 0 the ratio is unity within 3 \% at every lag from 1 to 800 on both species. Out of equilibrium three things are established. First, a single effective temperature exists and equals $T$ over a window that shrinks with drive: FDT holds to within 10 \% up to t $\approx$ 220 at $\chi$ = 0.5, 37 at $\chi$ = 1 and 13 at $\chi$ = 1.5, that is, for frequencies above 1/$t_{\rm FDT}$($\chi$) with $t_{\rm FDT}$ falling roughly as $\chi ^{-2}$. Second, below that frequency the ratio grows as ($T_{\rm eff}$/T $-$ 1) $\propto$ $t^{1.0}$ at $\chi$ = 1 and 1.5 without a plateau to t = 800; the $t^{0.89}$ growth of the species-coordinate measurement is this regime. Third, \textbf{the two species share the same $T_{\rm eff}$(t) at every lag}, to 2 \% at t = 400 and 10 \% at t = 800, although their bare mobilities differ by a factor of 1.5. The single-temperature question therefore has an answer on independent coordinates: one temperature, shared, equal to the bath temperature above a cutoff frequency, and a shared function of the observation time rather than a number below it. The violation lives entirely at frequencies below 1/$t_{\rm FDT}$($\chi$), which places the dissipation of Section 3.4 in cluster-scale motion rather than at contact scale; we did not evaluate the Harada--Sasa sum rule that would make that quantitative.
...
About half of the growth on that coordinate is the aggregate translating under a net internal force --- a finite-size, window-limited effect rather than a new phenomenon (its amplitude falls roughly as $N^{-1/2}$ and it decorrelates over long runs) --- and the residue is internal. The single-particle measurement above contains the same physics without the drift confound, since a random-sign force pattern exerts no net force on the aggregate. The large--small difference on species coordinates reported earlier should be discarded for the reason given above.

\textbf{Interpretation: tier membership is frequency-dependent, and the boundary is measured.} We set out expecting these three diagnostics to place the system in a tier. Two of them cannot, for reasons of construction, and the third places it in two tiers at once, separated by a frequency.

The quadratic dissipation is real and cleanly measured, but weakly diagnostic on its own: above the threshold the quadratic form is built into the construction, and below it there is nothing to fit. What it establishes is a threshold in $\chi$ shared with the structural transition, and a validation of the configurational estimator. The Onsager measurement returned a stable, plateau-tested antisymmetry that the dissimilar-channel control shows to be most parsimoniously a property of how we built the second channel; it does not survive as a physical reciprocity relation. The effective temperature, measured on independent coordinates, is the one that returns a definite answer, and it is not the negative reported in an earlier version of this work: a single effective temperature equal to $T$ exists, shared by both species, at all frequencies above 1/$t_{\rm FDT}$($\chi$), and a shared, growing violation below.

The honest summary is that \textbf{the near-equilibrium description applies above a cutoff frequency that falls as the drive grows, and fails below it}, with the failure carried by cluster-scale motion. The earlier reading of these three diagnostics as placing the system in the linear-response tier did not survive the dissimilar-channel control; the subsequent reading, that no tier-II signature exists, did not survive a measurement on conjugate coordinates. We state the position the measurements support: FDT with a shared temperature at contact and intra-cluster timescales, and a growing, shared violation at cluster-motion timescales, with the boundary between them a measured function of $\chi$.
...
$|$z$|$ = 2.2 of eighteen). No pair of these observables traces a cycle at any lag. And in the cluster-size coordinate itself, what the condensate sheds and what it reabsorbs have the same size distribution at all six $\chi$ (two-sample KS p $\ge$ 0.53), so the shedding--reabsorption loop carries no circulating current either. ``No circulation in any projection examined'' therefore stands with three more projections examined, one of them the natural coordinate of the structural transition.
...
\item No circulation is detectable in the coarse network observables examined --- nor in an all-pairs lag test on seven observables, nor in the cluster-size coordinate, nor, by parity, in any spatial pseudoscalar --- yet the excess dissipation over the reciprocal control is positive and resolved. It is zero within error for $\chi$ $\le$ 0.25 and quadratic from $\chi$ = 0.75 to 8, with nine tenths of the antisymmetric push screened by contact forces above that threshold and all of it below; the negative linear term reported in an earlier version is this screening, quadratic and structural. The dynamics are genuinely irreversible; that irreversibility did not appear in any projection we examined, which is a statement about those projections rather than about the dynamics, and the antisymmetric pair law is itself 99.4 \% gradient on the contact graph.
...
\item No finite characteristic cluster size is selected: the largest cluster grows as $N^{1.00\pm0.07}$ across a fourfold range of system size, and the largest-cluster fraction is flat (0.79, range 0.71--0.86) across that range without any fit. The scale that is selected, $S^*$ $\approx$ 8--10 particles, is a \textbf{critical nucleus}: the Becker--D\"oring size drift changes sign at 10.1 (9.7--10.7) and the committor crosses one half at 8.0--8.8, with the longest-lingering clusters at 8--12. It is independent of $N$, of density within a band, and of $\chi$ above the threshold. The finite-cluster state is a long-lived transient whose lifetime is measured, not extrapolated: the condensation time scales as $N^{1.0}$ and the largest-cluster-fraction curves collapse in t/N over a fourfold range of $N$.
\item \textbf{Near-equilibrium behaviour has a measured frequency of validity.} Of the three diagnostics we tested, entropy production is quadratic above the structural threshold but that form is close to guaranteed by the construction of $\chi$; a differential cross-response between two nonreciprocal channels is predominantly antisymmetric when the channels share a structure, but a structurally dissimilar control inverts the pattern, so we withdraw the reciprocity reading. The third, measured on single-particle coordinates with a random-sign force conjugate to the single-particle fluctuation, calibrates to unity within 3 \% at every lag at $\chi$ = 0 and, out of equilibrium, gives a single effective temperature equal to $T$ shared by both species above a cutoff frequency 1/$t_{\rm FDT}$($\chi$) --- $t_{\rm FDT}$ = 216, 37 and 13 at $\chi$ = 0.5, 1 and 1.5 --- with a shared violation growing as t below it. The earlier verdict that no effective temperature exists was an artefact of a non-conjugate pairing on species coordinates. Whatever variational description applies, its scope is at most the dynamics at fixed structure above that frequency, since structure selection is a rate-balance fixed point that no functional tested here reproduces.
...
\item For the Network-Weighted Action Principle specifically, the structural proposal survives and its quantitative prediction is borne out though non-monotonically, while the mechanism attributed to the antisymmetric sector --- solenoidality, circulation, and a modularity signature --- fails on all three counts. The sector's measurable contribution is a positive excess dissipation above a structural threshold, quadratic there and zero below, with nine tenths of the push screened by contact forces throughout. That form is naturally represented by a dissipation functional; it does not support the stronger claim that the antisymmetric sector merely adds a structure-preserving term to an equilibrium action. The structure the sector selects is a rate-balance fixed point --- fragment count equal to shedding rate over reabsorption probability at every $\chi$ --- which is the concrete thing a functional for structure selection would have to reproduce.
\item In published social-dominance matrices, where the symmetric/antisymmetric decomposition is exact and finite-dimensional rather than inferred, the measured antisymmetric coupling is consistent with a pure gradient and shows no detectable circulating component --- though at an interaction density per ordered pair where the measurement has limited power, so this is ``consistent with a gradient'', not a demonstration of one. It none the less points the same way as the colloid result, by an independent route: antisymmetry does not imply circulation. On the colloid's own contact graph the same decomposition is exact and gives a 99.4 \% gradient at every $\chi$. Separately, the Hodge decomposition of dominance must be computed on log-odds: on raw counts a perfectly transitive group reads as 16\% intransitive however much data is collected, which is a floor that published intransitivity claims should be checked against.
