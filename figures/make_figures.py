...
def fig1_construction():
    ...
    fig.tight_layout(); fig.savefig("figures/fig1_construction.png", bbox_inches="tight")
    print("  figures/fig1_construction.png")


# ---------------------------------------------------------------- Fig 2
def fig2_structure():
    """Cluster count and largest-cluster fraction vs chi, showing the non-monotonic dip."""
    d = pd.read_csv("results/paper_results.csv")
    ...
    fig.tight_layout(); fig.savefig("figures/fig2_structure.png", bbox_inches="tight")
    print("  figures/fig2_structure.png")
...
def fig3_scaling():
    ...
    fig.tight_layout(); fig.savefig("figures/fig3_scaling.png", bbox_inches="tight")
    print("  figures/fig3_scaling.png")
...
def fig4_rates():
    ...
    from analysis.cluster_kinetics import BINS
    ev = pd.read_csv("results/kinetics_events.csv"); ev = ev[(ev.chi == 1.5) & (ev.S >= 2)]
    ...
    g = g[g.S < 100]
    d3 = pd.read_csv("results/committor_T3000.csv"); bins = [3, 4, 5, 6, 7, 8, 10, 12, 15, 20, 30, 1000]
    ...
    fig.tight_layout(); fig.savefig("figures/fig4_rates.png", bbox_inches="tight")
    print("  figures/fig4_rates.png")
...
def fig5_thermo():
    ...
    fig, ax = plt.subplots(1, 3, figsize=(7.2, 2.3))
    cal = pd.read_csv("results/epr_static_calibtraj.csv").groupby("chi").epr_calib.agg(["mean", "sem"])
    st = pd.read_csv("results/epr_static.csv"); st = st[(st.N == 4000) & (st.L == 24) & (st.chi >= 3)]
    ...
    hs = pd.read_csv("results/hs_results.csv"); hs = hs[hs.species.isin(["L", "S"]) & hs.t.notna()]
    ...
    fig.tight_layout(); fig.savefig("figures/fig5_thermo.png", bbox_inches="tight")
    print("  figures/fig5_thermo.png")
...
def fig6_circulation():
    ...
    fig.tight_layout(); fig.savefig("figures/fig6_circulation.png", bbox_inches="tight")
    print("  figures/fig6_circulation.png")
